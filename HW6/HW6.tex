...
\textbf{(10 points)} Prove the following statement:

\begin{theo}
	\textit{The square of any integer has the form $4k$ or $4k + 1$ for some integer $k$.}
	\begin{proof}
		Suppose $k$ is any integer.\\
		For any integer $k$, $k^2$ is its square.\\
...
\textbf{(10 points)} Show that the following statement is false:

``For all integers $a$ and $b$, if $3 \mid (a + b)$, then $3 \mid (a - b)$.''
\begin{theo}
	\textit{For all integers $a$ and $b$, if $3 \mid (a + b)$, then $3 \mid (a - b)$.}\\
...
\textbf{(10 points)} Prove that if $n$ is any nonnegative integer whose decimal representation ends in 5, then $5 \mid n$.
\begin{theo}
...
\begin{theo}
	\textit{Given any integer $n > 3$, could $n$, $n + 2$, and $n + 4$ all be prime?}
	\begin{proof}
		Suppose $n$ is any integer with $n > 3$.\\
		By the quotient remainder theorem, $n = 3q$, or $n = 3q + 1$ or $n = 3q +2$ for some integer $q$.
